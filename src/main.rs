...
fn get_factorio_filename(url: &str) -> AppResult<String> {
    let filename = url.split('/').last().ok_or_not_found("no '/' found")?;
    let filename = filename.split('?').next().ok_or_not_found("no '?' found")?;
    Ok(filename.to_string())
}

fn get_factorio_hash(filename: &str) -> AppResult<String> {
    let hashes = http::fetch(CHECKSUM_URL)?;
    let sha256 = hash::find_file_hash(filename, hashes)?;
    Ok(sha256)
}

fn get_factorio(url: &str, filename: &str, hash: &str) -> AppResult<()> {
...
fn execute_command(args: &Args) -> AppResult<()> {
    let exec = args.exec();
    if exec.is_empty() {
        return Ok(());
    }
...
fn update_factorio(args: &Args) -> AppResult<()> {
    let url = http::get_redirect_url(REDIRECT_URL)?;
    let filename = get_factorio_filename(&url)?;
    let hash = get_factorio_hash(&filename)?;
    get_factorio(&url, &filename, &hash)?;
